...
parameters {
  real log_q; // overall catchability
  ...
  real midpoint_01; // midpoint of sigmoid fishing days curve
  real slope_01; // slope of sigmoid fishing days curve
}
...
transformed parameters {
  ...
  real midpoint = 16 + (inv_logit(midpoint_01) * 4);
  real slope = inv_logit(slope_01)*20;
  real q = exp(log_q);
  ...
  for (i in 1:tmax) {
    ...
    if (i < tmax) {
      N[i + 1] = Np[i] * exp(-q * fdays(slope, i, dmax, midpoint) * n_boats[i]);
    }
    for (j in 1:n_fisher) {
